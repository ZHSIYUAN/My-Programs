...
int main(int argc, char* argv[]){
    ...
    try{
        ...
        int labeladdress = 0;
        int number_label = 0;
        vector<vector<Token*> >::iterator testit3;
        for(testit3 = tokLines.begin(); testit3 != tokLines.end(); ++testit3){
            vector<Token*>::iterator testit4;
            for(testit4 = testit3->begin(); testit4 != testit3->end(); ++testit4){
                if ((*testit4)->toString() == "DOTWORD") {
                    vector<Token*>::iterator myit = testit4;
                    ++myit;
                    if ((*myit)->toString() == "ID") {
                        ++number_label;
                        retable.push_back(labeladdress);
                    }
                    labeladdress = labeladdress + 4;
                    break;
                } else if ((*testit4)->toString() == "ID") {
                    string mystr = (*testit4)->getLexeme();
                    if (mystr == "bne" || mystr == "beq") {
                        vector<Token*>::iterator myit = testit4;
                        // determine if label add address to vector
                        ++myit;
                        ++myit;
                        ++myit;
                        ++myit;
        ...
        int length = labeladdress / 4;
        int cookie = 0x10000002;
        printword(cookie);
        int lgh = 12 + 4*length + 8*number_label;
        printword(lgh);
        int codelength = 12 + 4*length;
        printword(codelength);
        ...
        // start print relocate table
        int fix = 0x01;
        vector<int>::iterator reit;
        for (reit = retable.begin(); reit != retable.end(); ++reit) {
            printword(fix);
            int myint = *reit;
            //myint = myint + 12;
            printword(myint);
